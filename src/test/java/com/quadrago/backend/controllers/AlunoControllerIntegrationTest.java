...
@ActiveProfiles("test") // <-- 1. Adicionada anotação de perfil de teste
class AlunoControllerIntegrationTest {
    ...
    // 2. Testes descomentados para execução
    @Test
    @WithMockUser(roles = "ADMIN")
    ...
    void deveAtualizarAlunoComNovosProfessores() throws Exception {
        Professor p1 = professorRepository.save(new Professor(null, "Carlos", "11999999999", "11111111111", new HashSet<>()));
    ...
    void deveListarAlunosComProfessores() throws Exception {
        ...
        MvcResult result = mockMvc.perform(get("/aluno").with(jwt().authorities(new SimpleGrantedAuthority("ROLE_ADMIN"))))
                .andDo(print())
                .andExpect(status().isOk())
                .andReturn();
